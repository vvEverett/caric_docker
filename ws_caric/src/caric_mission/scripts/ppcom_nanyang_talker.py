...
if __name__ == '__main__':
    ...
    rospy.init_node('nanyang_talker', anonymous=False)
    ...
    response = create_ppcom_topic('nanyang', ['all'], '/ping_message', 'std_msgs', 'String')
    print(f"Response {response}")

    # Create the publisher
    msg_pub = rospy.Publisher('/ping_message', String, queue_size=1)
    ping_message_sub = rospy.Subscriber('/ping_message/nanyang', String, PingMessageCallback)

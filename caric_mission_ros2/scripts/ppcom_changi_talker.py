...
class ChangiTalker(Node):
    """Changi node talker for PPCom communication"""

    def __init__(self):
        super().__init__('changi_talker')
        ...
        # Create publisher
        self.msg_pub = self.create_publisher(String, '/changi/ping_message', 10)
        
        # Create subscribers for other nodes
        self.jurong_sub = self.create_subscription(
            String,
            '/domain4/jurong/ping_message',
        ...
        self.get_logger().info("Changi talker node started!")

    def register_topic(self):
        """Register the ping_message topic with PPCom router"""
        request = CreatePPComTopic.Request()
        request.source = 'changi'
        request.targets = ['all']
        request.topic_name = '/changi/ping_message'
    ...
    def timer_callback(self):
        ...
        msg = String()
        msg.data = f"test;/changi;{result_str}"
...
def main(args=None):
    ...
    changi_talker = ChangiTalker()
    
    try:
        rclpy.spin(changi_talker)
    except KeyboardInterrupt:
        pass
    finally:
        changi_talker.destroy_node()
        rclpy.shutdown()
